...
async function gandu() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      ...
      console.log(`${endi}`)
      console.log(chalk.bgBlack(chalk.redBright('initializing SNIPER Bot')))
    }
  } catch (error) {
    console.error('Error:', error)
  }
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    let msgf = `Hai🤩${name} Congrats you have successfully deployed SNIPER-BOT\nJoin my support Group for any Question\n https://chat.whatsapp.com/KkMsf03DcKjGRasACd3Se6`
