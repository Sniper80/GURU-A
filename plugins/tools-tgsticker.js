...
async function Telesticker(url) {
  return new Promise(async (resolve, reject) => {
    ...
    const data = await axios(
      `https://api.telegram.org/bot7485822834:AAEuxRqc835xQDcu3gwHreoEZB16mlsSkqI/getStickerSet?name=${encodeURIComponent(packName)}`,
    ...
    for (let i = 0; i < data.data.result.stickers.length; i++) {
      ...
      const data2 = await axios(
        `https://api.telegram.org/bot7485822834:AAEuxRqc835xQDcu3gwHreoEZB16mlsSkqI/getFile?file_id=${fileId}`
      )
      const result = {
        status: 200,
        author: 'Xfarr05',
        url: `https://api.telegram.org/file/bot7485822834:AAEuxRqc835xQDcu3gwHreoEZB16mlsSkqI/${data2.data.result.file_path}`,
      }
      hasil.push(result)
    }
    resolve(hasil)
