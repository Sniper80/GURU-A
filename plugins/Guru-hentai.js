let handler = async (m, { conn, text, usedPrefix, command, args }) => {
  ...
  if (user < 17) throw m.reply(`❎ u need to be atleast 18 years`)
  if (!text) throw `*This command provides sauce from nhentai: ${usedPrefix + command} miku*`
  try {
    m.reply(global.wait)
    let res = await fetch(
      `https://api.lolhuman.xyz/api/nhentaiseuarch?apikey=${lolkeysapi}&query=${text}`
    )
    let json = await res.json()
    let aa = json.result[0].id
    let aa2 = json.result[0].title_native
    let res2 = await fetch(`https://api.lolhuman.xyz/api/nhenthaipdf/${aa}?apikey=${lolkeysapi}`)
